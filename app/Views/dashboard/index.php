...
.stat-card.total { background:#2E86C1; }
.stat-card.today { background:#1ABC9C; }
.stat-card.week { background:#F39C12; }
.stat-card.month { background:#8E44AD; }
.stat-card.section { background:#6c757d; }
.stat-card.section-today { background:#20c997; }
...
<style>
:root{
  --bg:#f1f3f5; --card:#fff; --muted:#6b7280; --accent:#2E86C1; --text:#111827;
}
[data-theme="dark"]{
...
/* small icon button */
.icon-btn { background:transparent; border:1px solid rgba(0,0,0,0.06); padding:8px 10px; border-radius:8px; cursor:pointer; color:var(--text); }
.icon-btn:hover { box-shadow:0 3px 8px rgba(0,0,0,0.08); }

/* counter style (keeps previous card styles) */
.stat-card .count { font-size:28px; font-weight:700; }

/* leaderboard */
.leaderboard { background:var(--card); padding:12px; border-radius:10px; box-shadow:0 2px 8px rgba(0,0,0,0.06); }
.leaderboard h4 { margin:0 0 8px 0; font-size:15px; }
.leaderboard ol { padding-left:18px; margin:0; color:var(--muted); }

/* small print friendly tweak for PDF export */
@media print {
  .sidebar, .icon-btn, .filters-box, .header-actions { display:none !important; }
...
<div class="dashboard-container">
    ...
    <div class="main">
        ...
        <div class="cards-grid">
            <div class="stat-card total"><div class="count"><?php echo $stats['total']; ?></div>Total Passes</div>
            <div class="stat-card today"><div class="count"><?php echo $stats['today']; ?></div>Today</div>
            <div class="stat-card week"><div class="count"><?php echo $stats['week']; ?></div>Last 7 Days</div>
            <div class="stat-card month"><div class="count"><?php echo $stats['month']; ?></div>This Month</div>
            <div class="stat-card section"><div class="count"><?php echo $stats['section_total']; ?></div>Section Total</div>
            <div class="stat-card section-today"><div class="count"><?php echo $stats['section_today']; ?></div>Section Today</div>
        </div>
        ...
        <div class="table-box">
            ...
            <div class="table-responsive">
                <table class="table-modern">
                    <thead>
                        <tr>
                            <th>ID</th><th>Pass No</th><th>CINO</th>
                            <th>Date</th><th>For</th><th>Adv</th><th>Court/Item</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if(empty($rows)): ?>
...
(function(){
  ...
  function startAutoRefresh(){
    ...
    autoInterval = setInterval(function(){ location.reload(); }, 45000);
  }
